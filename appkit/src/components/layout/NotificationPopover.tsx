...
import { Bell, Check, Clock, Activity } from 'lucide-react'
...
export function NotificationPopover({ user }: NotificationPopoverProps) {
  ...
  return (
    <Menu as="div" className="relative">
      ...
      >
        <Menu.Items className="absolute right-0 mt-4 w-80 sm:w-96 origin-top-right rounded-2xl bg-white/95 dark:bg-zinc-900/95 backdrop-blur-xl shadow-2xl ring-1 ring-black/5 dark:ring-white/5 focus:outline-none z-50 border border-gray-200/50 dark:border-zinc-800/50 overflow-hidden">
            ...
            {notifications.length === 0 ? (
              ...
              <div className="divide-y divide-gray-100/50 dark:divide-zinc-800/50">
                {notifications.map((notif, index) => (
                  <Menu.Item key={index}>
                    {({ active }) => (
                      <div className={`p-4 transition-colors ${active ? 'bg-gray-50/80 dark:bg-zinc-800/80' : ''} ${index < unreadCount ? 'bg-blue-50/30 dark:bg-blue-500/5' : ''}`}>
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">
                              {formatAction(notif.action || 'system_event')}
                            </p>
                            <p className="text-xs text-gray-600 dark:text-zinc-400 mt-0.5 leading-snug line-clamp-2">
                              {notif.details ? JSON.stringify(notif.details) : 'Admin action recorded in the system audit logs.'}
                            </p>
