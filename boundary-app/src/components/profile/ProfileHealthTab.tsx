            ...
            <View style={styles.section}>
                ...
                {subCats.length === 0 ? (
                    <View style={styles.emptySubCats}>
                        ...
                        <TouchableOpacity style={styles.emptySubCatsAction} onPress={() => setShowManageSubCats(true)}><Text style={[styles.emptySubCatsActionText, { color }]}>Add sub-category</Text></TouchableOpacity>
                    </View>
                ) : (
                                ...
                                <View key={sc.id} style={styles.subCatCard}>
                                    ...
                                    {isOpen && (
                                        <View style={styles.subCatContent}>
                                            ...
                                            <TouchableOpacity style={[styles.addRecordInline, { borderColor: color }]} onPress={() => openAddRecord(sc.id)}><IconMC name="plus" size={14} color={color} /><Text style={[styles.addRecordInlineText, { color }]}>Add Record</Text></TouchableOpacity>
                                        </View>
                                    )}
                                </View>
                            );
    ...
    return (
        <View style={styles.container}>
                        ...
                        <View style={styles.modalHeader}>
                            ...
                            {addRecordSubCat && selectedCategory && (
                                <View style={styles.subCatBadgeRow}>
                                    <View style={[styles.catBadge, { backgroundColor: `${selectedCategory.color}15` }]}><IconMC name={selectedCategory.icon || 'tag'} size={12} color={selectedCategory.color} /><Text style={[styles.catBadgeText, { color: selectedCategory.color }]}>{selectedCategory.name}</Text></View>
                                    <IconMC name="chevron-right" size={12} color="#94A3B8" />
                                    <View style={[styles.catBadge, { backgroundColor: '#F1F5F9' }]}><Text style={styles.subCatBadgeText}>{addRecordSubCat.name}</Text></View>
                                </View>
                            )}
                        </View>
            ...
            <Modal visible={showManageSubCats} transparent animationType="slide" onRequestClose={() => setShowManageSubCats(false)}>
                ...
                <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalOverlay} pointerEvents="box-none">
                    <View style={styles.manageSheet}>
                        <View style={styles.modalHandle} /><View style={styles.modalHeader}><Text style={styles.modalTitle}>Manage Sub-categories</Text>{selectedCategory && (<View style={[styles.catBadge, { backgroundColor: `${selectedCategory.color}15` }]}><IconMC name={selectedCategory.icon || 'tag'} size={12} color={selectedCategory.color} /><Text style={[styles.catBadgeText, { color: selectedCategory.color }]}>{selectedCategory.name}</Text></View>)}</View>
                        <ScrollView style={styles.manageSubCatList} showsVerticalScrollIndicator={false}>
                            {selectedCategory && (subCatsByCategory[selectedCategory.id] || []).length === 0 && (<View style={styles.emptySubCats}><Text style={styles.emptySubCatsText}>No sub-categories yet</Text></View>)}
                            {selectedCategory && (subCatsByCategory[selectedCategory.id] || []).map(sc => (
                                <View key={sc.id} style={styles.manageSubCatRow}><View style={[styles.subCatDot, { backgroundColor: selectedCategory.color, width: 8, height: 8 }]} /><Text style={styles.manageSubCatName}>{sc.name}</Text><Text style={styles.manageSubCatCount}>{(recordsBySubCat[sc.id] || []).length} records</Text><TouchableOpacity onPress={() => handleDeleteSubCatSafe(sc.id)} style={styles.deleteSubCatBtn}><IconMC name="delete-outline" size={18} color="#EF4444" /></TouchableOpacity></View>
                            ))}
                        </ScrollView>
                        <View style={styles.addSubCatRow}><TextInput style={styles.addSubCatInput} value={newSubCatName} onChangeText={setNewSubCatName} placeholder="New sub-category name" placeholderTextColor="#94A3B8" returnKeyType="done" onSubmitEditing={handleAddSubCat} /><TouchableOpacity style={[styles.addSubCatBtn, !newSubCatName.trim() && styles.addSubCatBtnDisabled]} onPress={handleAddSubCat} disabled={!newSubCatName.trim()}><IconMC name="plus" size={20} color={!newSubCatName.trim() ? '#94A3B8' : '#FFFFFF'} /></TouchableOpacity></View>
                        <View style={styles.modalActions}><TouchableOpacity style={[styles.cancelBtn, { flex: 0, paddingHorizontal: 32 }]} onPress={() => setShowManageSubCats(false)}><Text style={styles.cancelBtnText}>Done</Text></TouchableOpacity></View>
                    </View>
                </KeyboardAvoidingView>
            </Modal>
            ...
            <Modal visible={showAddCategory} transparent animationType="slide" onRequestClose={() => setShowAddCategory(false)}>
                ...
                <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalOverlay} pointerEvents="box-none">
                    <View style={styles.modalSheet}>
                        <View style={styles.modalHandle} /><View style={styles.modalHeader}><Text style={styles.modalTitle}>New Category</Text><TouchableOpacity onPress={() => setShowAddCategory(false)}><IconMC name="close" size={20} color="#94A3B8" /></TouchableOpacity></View>
                        <View style={styles.modalBody}>
                            <Text style={styles.modalFieldLabel}>Type</Text>
                            <View style={styles.catTypeRow}>{addCatSection === 'flow' ? (<>{[{ v: 'input', label: 'Input', color: '#10B981' }, { v: 'output', label: 'Output', color: '#EF4444' }].map(opt => (<TouchableOpacity key={opt.v} style={[styles.catTypeChip, addCatType === opt.v && { backgroundColor: opt.color, borderColor: opt.color }]} onPress={() => setAddCatType(opt.v)}><Text style={[styles.catTypeChipText, addCatType === opt.v && { color: '#FFFFFF' }]}>{opt.label}</Text></TouchableOpacity>))}</>) : (<View style={[styles.catTypeChip, styles.catTypeChipActive]}><Text style={[styles.catTypeChipText, { color: '#FFFFFF' }]}>{addCatSection === 'assets' ? 'Positive' : 'Negative'}</Text></View>)}</View>
                            <Text style={[styles.modalFieldLabel, { marginTop: 16 }]}>Name</Text>
                            <TextInput style={styles.modalInput} value={addCatName} onChangeText={setAddCatName} placeholder={addCatSection === 'assets' ? 'e.g. Exercise' : addCatSection === 'liabilities' ? 'e.g. Bad Habits' : addCatType === 'input' ? 'e.g. Cardio' : 'e.g. Stress'} placeholderTextColor="#94A3B8" autoFocus />
                        </View>
                        <View style={styles.modalActions}><TouchableOpacity style={styles.modalCancelBtn} onPress={() => setShowAddCategory(false)}><Text style={styles.modalCancelText}>Cancel</Text></TouchableOpacity><TouchableOpacity style={[styles.modalSaveBtn, (!addCatName.trim() || addCatSaving) && styles.modalSaveBtnDisabled]} onPress={handleAddCategory} disabled={!addCatName.trim() || addCatSaving}>{addCatSaving ? <ActivityIndicator size="small" color="#FFFFFF" /> : <Text style={styles.modalSaveText}>Create</Text>}</TouchableOpacity></View>
                    </View>
                </KeyboardAvoidingView>
            </Modal>
            ...
            <Modal visible={showMoveDrawer} transparent animationType="slide" onRequestClose={() => !moveWorking && setShowMoveDrawer(false)}>
                ...
                <View style={styles.modalOverlay} pointerEvents="box-none">
                    <View style={styles.moveSheet}>
                        <View style={styles.modalHandle} />
                        <View style={styles.moveSheetHeader}><View><Text style={styles.modalTitle}>{moveType === 'subcategory' ? 'Delete Sub-category' : 'Delete Category'}</Text><Text style={styles.moveSheetSubtitle}>{moveSourceRecordCount} record{moveSourceRecordCount !== 1 ? 's' : ''} · {formatScore(moveSourceAmount)}</Text></View><TouchableOpacity onPress={() => setShowMoveDrawer(false)} disabled={moveWorking}><IconMC name="close" size={20} color="#94A3B8" /></TouchableOpacity></View>
                        <View style={styles.movePanels}>
                            <View style={styles.movePanel}><Text style={styles.movePanelLabel}>FROM</Text><View style={[styles.movePanelCard, { borderColor: moveType === 'subcategory' ? moveSourceSubCat?.cat.color || '#E2E8F0' : moveSourceCat?.color || '#E2E8F0' }]}><View style={[styles.movePanelDot, { backgroundColor: moveType === 'subcategory' ? moveSourceSubCat?.cat.color || '#64748B' : moveSourceCat?.color || '#64748B' }]} /><View style={{ flex: 1 }}><Text style={styles.movePanelName} numberOfLines={2}>{moveType === 'subcategory' ? moveSourceSubCat?.sc.name : moveSourceCat?.name}</Text>{moveType === 'subcategory' && moveSourceSubCat && (<Text style={styles.movePanelSub} numberOfLines={1}>{moveSourceSubCat.cat.name}</Text>)}<Text style={styles.movePanelMeta}>{moveSourceRecordCount} records</Text><Text style={styles.movePanelAmount}>{formatScore(moveSourceAmount)}</Text></View></View></View>
                            <View style={styles.moveArrowCol}><IconMC name="arrow-right-bold" size={22} color={moveDestId ? '#0F172A' : '#CBD5E1'} /></View>
                            <View style={styles.movePanel}><Text style={styles.movePanelLabel}>TO</Text>
                                <ScrollView style={styles.moveDestList} showsVerticalScrollIndicator={false}>
                                    {moveDestOptions.length === 0 ? (<View style={styles.moveDestEmpty}><Text style={styles.moveDestEmptyText}>No other options</Text></View>) : (
                                        moveDestOptions.map(opt => {
                                            const selected = moveDestId === opt.id;
                                            return (
                                                <TouchableOpacity key={opt.id} style={[styles.moveDestItem, selected && styles.moveDestItemSelected]} onPress={() => setMoveDestId(opt.id)} activeOpacity={0.7}><View style={[styles.movePanelDot, { backgroundColor: opt.color, width: 7, height: 7 }]} /><View style={{ flex: 1 }}><Text style={[styles.moveDestItemName, selected && styles.moveDestItemNameSelected]} numberOfLines={1}>{opt.label}</Text><Text style={styles.moveDestItemSub} numberOfLines={1}>{opt.sublabel}</Text></View>{selected && <IconMC name="check-circle" size={15} color="#0F172A" />}</TouchableOpacity>
                                            );
                                        })
                                    )}
                                </ScrollView>
