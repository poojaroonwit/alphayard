...
const DEFAULT_APP_SLUG = 'boundary';
 ...
 */
export async function getBoundaryApplicationId(): Promise<string | undefined> {
    return getDefaultAppId();
}

/**
 * Get the default application ID (boundary)
...
export async function getDefaultAppId(): Promise<string | undefined> {
    ...
    // Query database - try 'boundary' (new) and 'bondarys' (legacy) slugs
    const application = await prisma.application.findFirst({
        where: {
            slug: {
                in: ['boundary', 'bondarys']
            },
            isActive: true
        },
        orderBy: {
